...
13. **IP registrada si se evita Netlify:** la API también responde en la URL directa de Render. Quien la llame directamente, sin pasar por Netlify, puede agregar una IP falsa en `X-Forwarded-For`, y la bitácora registraría esa IP. Las demás protecciones siguen activas (verificación de origen, sesión, roles y segundo factor); el riesgo afecta solo a la exactitud de la IP en ese caso.
