...
|---|---|---|---|
| **D-01** | Despliegue para el grupo auditor | **Resuelta (25/09/2026):** frontend en **Netlify**, API en **Render** y base en **MongoDB Atlas**. Netlify reenvía `/api/*` a Render para que el navegador vea un solo dominio (cookie `SameSite=Strict`, CSP `'self'`, WebAuthn con el dominio de Netlify). Configuración en `netlify.toml` y `render.yaml`. | — |
...
| **D-03** | Correo para re-autenticación | **Resuelta (25/09/2026):** las operaciones sensibles aceptan **huella o código por correo** (`STEP_UP_ALLOW_EMAIL=true`). La huella es la primera opción y el correo la alternativa, para quien no tenga lector. El entregable debe decirlo así (conflicto C-03). | — |
...
| **D-06** | Servidor de correo | **Resuelta (25/09/2026):** en desarrollo `MAIL_TRANSPORT=console` muestra los correos en la terminal del backend. En producción se usa la **API HTTPS de Brevo** (`MAIL_TRANSPORT=brevo`), porque Render gratis bloquea el SMTP desde septiembre de 2025. | — |
...
| **D-20** | Lista de contraseñas prohibidas | Incluir una lista de contraseñas comunes (≈10 000) en `farmacentro-backend/src/data/commonPasswords.ts`, como recomienda NIST SP 800-63B-4. | ¿La incluimos? |
